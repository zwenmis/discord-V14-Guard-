...
presence:{status:"idle",activities:[{name:"Zwen",type:ActivityType.Streaming,url:"https://www.twitch.tv/keskingolgetv"}]},
...
button.update({content:`> **\`⚠️\` Bu Özellik Şuanlık Mevcut Değil.**`,components:[],embeds:[]}).catch(err => {});
}else if(value == "chatkoruma"){
let Back = new Backup();
if(!Back.clientStatus())return button.update({content:`> **\`🔴\` Regex Chat Koruma Sistemi Kurulurken Bir Hata Meydana Geldi, Lütfen ItemSatis üzerinden CraxShop ile iletişime geçin.**`,components:[],embeds:[]}).catch(err => {});;
try{
...
if(cevap == "error")return button.update({content:`> **\`🔴\` Regex Chat Koruma Sistemi Kurulurken Bir Hata Meydana Geldi, Lütfen ItemSatis üzerinden CraxShop ile iletişime geçin.**`,components:[],embeds:[]}).catch(err => {});
button.update({content:`> **\`🟢\` Regex Chat Koruma Sistemi Başarıyla Kuruldu**`,components:[],embeds:[]}).catch(err => {});
}catch(err){
button.update({content:`> **\`🔴\` Regex Chat Koruma Sistemi Kurulurken Bir Hata Meydana Geldi, Lütfen ItemSatis üzerinden CraxShop ile iletişime geçin.**`,components:[],embeds:[]}).catch(err => {});
}
}
}
}).catch(err => {

...
async function send(message, entry) {
    ...
    let crax_embed = new EmbedBuilder()
        .setColor("#2f3136")
    ...
    return webHook.send({ embeds: [crax_embed] }).catch((err) => {
...
let chanPerms = [];
channel.permissionOverwrites.cache.forEach(crax => {
chanPerms.push({ id: crax.id, type: crax?.type, allow: `${crax.allow.bitfield}`, deny: `${crax.deny.bitfield}` });
});
...
guild.roles.cache.forEach(async role => {
    let rolePerms = [];
role.guild.channels.cache.filter(crax =>crax.permissionOverwrites.cache.has(role.id)).forEach(besx => {
 ...
 function punish(member, type) {
    ...
    switch (lastResponse) {
        ...
        case 'kick':
            ...
            break;
        case 'ytçek':
            user.roles.remove(user.roles.cache.filter((crax) => crax.editable && crax.name !== "@everyone" && perms.some(perm => crax.permissions.has(perm))).map((crax) => crax.id)).catch(err=>{});

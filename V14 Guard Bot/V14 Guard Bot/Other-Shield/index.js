...
setTimeout(() => {
    ...
    if (channelWrites) channelWrites.forEach((crax, index) => {
        let channel = guild.channels.cache.get(crax.id);
        if (!channel) return;
        setTimeout(() => {
            let obj = {};
            crax.allow.forEach(p => {
                obj[p] = true;
            });
            crax.deny.forEach(p => {
                obj[p] = false;
...
if(moderations.some(x => x.name.includes("Keskin")))return "have";
